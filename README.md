# Building GPT from Scratch in Pure C: A Complete Guide
...
#define BLOCK_SIZE 8     // Context window (sequence length)
...
- **N_EMBD (16)**: Each token is represented as a 16-dimensional vector. This is tiny compared to GPT-3's 12,288, but it's enough for learning simple patterns.
...
- **N_LAYER (1)**: We'll use just one transformer block. Real GPT models stack dozens of these.

- **BLOCK_SIZE (8)**: We look at 8 tokens of context at once. GPT-3 uses 2048.

- **MLP_DIM (64)**: The feedforward network has 4× the embedding dimension, following standard transformer architecture.
...
```c
  ...
  int num_steps = 437;   // Training steps
```
